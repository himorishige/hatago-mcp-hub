 ...
 */
async function handleConnect(request: Request, _env: Env): Promise<Response> {
...
async function handleCall(request: Request, env: Env): Promise<Response> {
  const { serverId, method, params, progressToken }: ConnectorRequest =
    await request.json();

  // Get server config from KV
  const serverConfig = await env.CONFIG_KV.get<ServerConfig>(
    `server:${serverId}`,
    'json',
  );
  if (!serverConfig) {
