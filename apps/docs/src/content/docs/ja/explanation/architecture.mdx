...
Hatagoには診断用の内部ツールが組み込まれています：

| ツール                          | 用途                     |
| ------------------------------- | ------------------------ |
| `_internal_hatago_status`       | 全サーバーの接続状態確認 |
| `_internal_hatago_reload`       | 設定の手動リロード       |
| `_internal_hatago_list_servers` | サーバー詳細一覧         |

<Aside type="note">
  これらのツールは`_internal_`プレフィックスで識別され、通常のMCPツールと区別されます。
</Aside>

## 📚 関連ドキュメント

<CardGrid>
  <Card title="MCPプロトコル" icon="document" href="/ja/explanation/mcp-protocol/">
    プロトコルの詳細
  </Card>
