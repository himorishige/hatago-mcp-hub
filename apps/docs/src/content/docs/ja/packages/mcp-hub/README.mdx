...
description: CLIツールとメインエントリーポイント
package: mcp-hub
lang: ja
---
...
hatago init --mode http        # HTTP モード（StreamableHTTP）用テンプレート
...
# 設定の自動リロードは `Reference › Config` の「設定の自動リロード」を参照
...
### HTTP モード (StreamableHTTP transport)
...
- **Performance**: 8.44x faster startup (v0.0.14)
