...
import { 
  ...
  type SSEStream
} from '@hatago/transport';
import { Logger } from './logger.js';
import { SSEManager, type ProgressNotification } from './sse-manager.js';
import { 
  SessionManager,
  ToolRegistry,
  ToolInvoker,
...
export class HatagoHub {
  ...
  constructor(options: HubOptions = {}) {
    ...
    this.toolInvoker = new ToolInvoker(this.toolRegistry, {
  ...
  private async connectServer(id: string, spec: ServerSpec): Promise<void> {
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes('-32601')) {
        ...
        this.logger.warn(`Failed to list resources for ${id}`, { error: error instanceof Error ? error.message : String(error) });
      }
    }
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes('-32601')) {
        ...
        // Other errors are still warnings
        this.logger.warn(`Failed to list prompts for ${id}`, { error: error instanceof Error ? error.message : String(error) });
      }
    }
  }
  ...
  async start(): Promise<this> {
    ...
    if (this.options.configFile) {
      try {
          ...
          for (const [id, serverConfig] of Object.entries(config.mcpServers)) {
            ...
            if (hatagoOptions.start !== 'lazy') {
              try {
                ...
                this.logger.error(`Failed to connect to server ${id}`, { error: error instanceof Error ? error.message : String(error) });
        ...
        this.logger.error('Failed to load config file', { error: error instanceof Error ? error.message : String(error) });
        throw error;
      }
    }
  ...
  tools = {
     ...
     * Call a tool
     */
    call: async (name: string, args: any, options?: CallOptions & { progressToken?: string; progressCallback?: any }) => {
      ...
      const publicName = serverId && this.options.namingStrategy !== 'none'
      ...
      if (resourceInfo) {
        ...
        if (client) {
          try {
            ...
            this.emit('resource:read', { uri, serverId: resourceInfo.serverId, result });
            return result;
          } catch (error) {
      ...
      if (options?.serverId) {
        ...
        if (client) {
          try {
            ...
            this.emit('resource:read', { uri, serverId: options.serverId, result });
            return result;
          } catch (error) {
            this.logger.error(`Failed to read resource ${uri}`, { 
  ...
  async handleHttpRequest(context: any): Promise<Response> {
    ...
    this.logger.debug('[Hub] HTTP request received', { method, url: url.toString() });
...
`);
   ...
   */
  private async handleJsonRpcRequest(body: any, sessionId?: string): Promise<any> {
          ...
          for (const [serverId, client] of this.clients.entries()) {
            try {
              ...
              if (templatesResult?.resourceTemplates) {
                ...
                const namespacedTemplates = templatesResult.resourceTemplates.map((template: any) => ({
                  ...template,
                  name: template.name ? `${serverId}${this.options.separator}${template.name}` : undefined,
                  serverId
                }));
              ...
              this.logger.debug(`Server ${serverId} doesn't support resource templates`, { 
                error: error instanceof Error ? error.message : String(error) 
              });
            }
          }
